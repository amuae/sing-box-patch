...
func (m *Manager) Remove(tag string) error {
	...
	if index == -1 {
		// Provider outbound may not be in the main outbound list
		// This is expected behavior for provider-managed outbounds
		m.logger.Debug("outbound ", tag, " not found in main outbound list, skipping removal from list")
	} else {
...
func (m *Manager) Create(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, inboundType string, options any) error {
	...
	if existsOutbound, loaded := m.outboundByTag[tag]; loaded {
		...
		if existsIndex == -1 {
			// Provider outbound may not be in the main outbound list
			m.logger.Debug("existing outbound ", tag, " not found in main outbound list")
		} else {
