...
# Sing-Box 1.13 版本补丁项目自动化编译脚本
# 用法: ./build-1.13.sh [sing-box源码路径] [选项]
...
# 版本配置
SING_BOX_VERSION="v1.13.0-alpha.15"  # 默认版本，支持auto自动查询
PATCH_VERSION="1.13"
...
query_latest_version() {
    local branch=$1
    print_info "查询 $branch 系列最新版本..."
    
    # 1.13系列只有alpha版本
    if [ "$branch" = "1.13" ]; then
        local version=$(curl -s "https://api.github.com/repos/SagerNet/sing-box/releases" | \
            jq -r '.[] | select(.prerelease == true and (.tag_name | test("^v1\\.13\\.0-alpha\\.[0-9]+$"))) | .tag_name' | \
            sort -V | tail -1)
    else
        # 其他版本优先查找release版本
        local version=$(curl -s "https://api.github.com/repos/SagerNet/sing-box/releases" | \
        ...
        # 如果没有release版本，查找最新的pre-release版本
        if [ -z "$version" ] || [ "$version" = "null" ]; then
            version=$(curl -s "https://api.github.com/repos/SagerNet/sing-box/releases" | \
                jq -r '.[] | select(.prerelease == true and (.tag_name | test("^v'"$branch"'\\.[0-9]+-"))) | .tag_name' | \
                sort -V | tail -1)
        fi
    fi
    ...
    if [ -n "$version" ] && [ "$version" != "null" ]; then
        ...
        print_error "未找到 $branch 系列的可用版本"
        echo ""
    fi
}
...
init_build_env() {
    ...
    if [ -d "$BUILD_DEV_DIR" ]; then
        print_info "清理现有编译环境: $BUILD_DEV_DIR"
        rm -rf "$BUILD_DEV_DIR"
    fi
    
    mkdir -p "$BUILD_DEV_DIR"
    print_success "编译环境初始化完成: $BUILD_DEV_DIR"
}
...
cleanup_build_env() {
    ...
    if [ -d "$BUILD_DEV_DIR" ]; then
        rm -rf "$BUILD_DEV_DIR"
        print_success "编译环境清理完成"
    fi
}

# 错误处理函数
cleanup_on_error() {
    print_error "编译过程中发生错误，正在清理..."
    cleanup_build_env
    exit 1
}
...
选项:
    ...
    -v, --version VERSION   指定 sing-box 版本 (默认: $SING_BOX_VERSION, 使用 'auto' 自动查询最新版本)
...
# 处理版本自动查询
if [[ "$SING_BOX_VERSION" == "auto" ]]; then
    print_info "自动查询最新版本..."
    AUTO_VERSION=$(query_latest_version "$PATCH_VERSION")
    if [[ -n "$AUTO_VERSION" ]]; then
        SING_BOX_VERSION="$AUTO_VERSION"
        print_success "自动选择版本: $SING_BOX_VERSION"
    else
        print_error "自动查询版本失败，请手动指定版本"
        exit 1
    fi
fi
...
if [[ -z "$SING_BOX_PATH" ]]; then
    ...
    SING_BOX_PATH="$BUILD_DEV_DIR/sing-box"
    ...
    # 如果指定了源码路径，复制到 build-dev 目录
    print_info "复制源码到编译环境..."
    cp -r "$SING_BOX_PATH" "$BUILD_DEV_DIR/sing-box"
    SING_BOX_PATH="$BUILD_DEV_DIR/sing-box"
    print_success "源码复制完成: $SING_BOX_PATH"
fi
...
# 检查是否有补丁文件存在
if [[ -d "$PATCH_DIR/source/$PATCH_VERSION" ]]; then
    # 使用源码文件方式
    print_info "使用源码文件方式应用补丁..."
    cp -r "$PATCH_DIR/source/$PATCH_VERSION"/* "$SING_BOX_PATH/"
    print_success "源码文件复制完成"
elif [[ -d "$PATCH_DIR/patches/$PATCH_VERSION" ]] && [[ -f "$PATCH_DIR/apply-patch.sh" ]]; then
    # 使用补丁文件方式
    print_info "使用补丁文件方式应用补丁..."
    cd "$PATCH_DIR"
    ./apply-patch.sh "$SING_BOX_PATH" "$PATCH_VERSION"
